...
export const findOrCreateThread = async (userId: string, customerId: string, threadId: string): Promise<string> => {
  try {
    ...
    const msg = `Failed to create thread with id ${threadId} for user ${userId}`;
    log.error({
      action: 'findOrCreateThread',
      result: 'failure',
      e: err.stack,
      msg,
    });
...
export const getThreadByUserId = async (userId: string): Promise<string> => {
  try {
